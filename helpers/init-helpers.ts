...
export const initReservesByHelper2 = async (
  ...
  for (let [symbol, params] of reserves) {
    ...
    initInputParams.push({
      ...
      stableDebtTokenSymbol: `stableDebt${symbolPrefix}${symbol}`,
      params: await getATokenExtraParams(aTokenImpl, tokenAddresses[symbol]),
      project: projectAddresses[`Project${symbol}`], // use to deploy in blockchain
      // project: projectAddresses[symbol],  // use to deploy in test env
...
export const configureReservesByHelper2 = async (
  ...
  for (const [
    ...
    if (!projectAddresses[`Project${assetSymbol}`]) {
      console.log(
        `- Skipping init of ${assetSymbol} due token address is not set at markets config`
      );
      continue;
    ...
    const assetAddressIndex = Object.keys(projectAddresses).findIndex(
      (value) => value === `Project${assetSymbol}`
    );
